...
class GroundingDINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List = None, **kw):
        ...
        return_backbone_features = kw.get("return_backbone_features", False)
        ...
        # output
        outputs_class = torch.stack([layer_cls_embed(layer_hs, text_dict) for layer_cls_embed, layer_hs in zip(self.class_embed, hs)])
        out = {"pred_logits": outputs_class[-1], "pred_boxes": outputs_coord_list[-1]}

        # Provide decoder refined queries (last layer) for downstream contrastive usage
        try:
            if isinstance(hs, (list, tuple)):
                out["decoder_queries"] = hs[-1]  # [B, N_q, C]
            elif isinstance(hs, torch.Tensor):
                # shape might be [num_layers, B, N_q, C]
                if hs.dim() == 4:
                    out["decoder_queries"] = hs[-1]  # last layer
                else:
                    out["decoder_queries"] = hs  # fallback
        except Exception:
            pass

        # Optionally return multi-scale backbone feature maps (projected to hidden_dim)
        if return_backbone_features:
            # srcs list already contains projected feature maps: [B, C, H_l, W_l]
            out["multi_scale_features"] = [s.detach() for s in srcs]
            # For compatibility with code paths expecting single-scale encoder_features
            if srcs:
                out["encoder_features"] = srcs[0].detach()
        ...
        unset_image_tensor = kw.get("unset_image_tensor", True) and (not return_backbone_features)
